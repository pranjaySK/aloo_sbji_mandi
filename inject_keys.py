...
# We will match the end of each Map definition which is typically:
#   };
# But to be precise, we know each map starts with `static const Map<String, String> _<lang> = {`
# We'll split the content by `};`
# However, this might match unexpected places if `};` occurs elsewhere.
# Safer way: Find `static const Map<String, String> _xxx = {` and then find the NEXT `};` at the start of a line or `  };`

new_content = ""
current_pos = 0

pattern = re.compile(r'(static const Map<String, String> _[a-z]{2} = \{.*?\n)   };', re.DOTALL)

def inject(match):
    # match.group() is the whole map up to the closing  };
    # match.group(1) is the map without `   };`
    return match.group(1) + chunk + "\n  };"

new_content = pattern.sub(inject, original_content)

print(f"Number of maps matched: {original_content.count('static const Map<String, String> _')}")
print(f"Number of maps modified: {len(pattern.findall(original_content))}")

# Check if replacements were actually made
if new_content != original_content:
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("app_localizations.dart injected successfully!")
else:
    print("Failed to inject: Regex did not match the map structure.")
