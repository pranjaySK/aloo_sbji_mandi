...
in_map = False
maps_modified = 0
existing_keys_in_current_map = set()
...
for line in lines:
    ...
    if in_map:
        ...
        if match:
            existing_keys_in_current_map.add(match.group(1))

    if in_map and line.strip() == "};":
        # We reached the end of the current map. Time to inject missing keys.
        missing_keys = {k: v for k, v in raw_keys.items() if k not in existing_keys_in_current_map}
        
        if missing_keys:
            new_lines.append("    // Admin Screens Localization Injected via Script\n")
            for k, v in missing_keys.items():
                escaped_val = v.replace("'", "\\'")
                new_lines.append(f"    '{k}': '{escaped_val}',\n")
...
print("Finished smarter injection.")
